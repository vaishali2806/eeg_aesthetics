...
%  2. Averages precomputed EEG RDMs across participants (18–39) at 20 Hz.
...
% Re‐code the 13 original styles into 3 groups:
%   group1 = [1,2,3,4,9], group2 = [5,6,7,10], group3 = [8,11,12,13]
group1 = [1, 2, 3, 4, 9];
group2 = [5, 6, 7, 10];
group3 = [8, 11, 12, 13];
...
% For each category (1–5) and each new style (1–3), build a binary RDM:
%   RDM(i,j) = 0 if same label, 1 if different label.
...
%%
%%====================== AVERAGE EEG RDMs ACROSS SUBJECTS ==================

n_participants = 39;
n_timebins = 110;
matrix_size = 999;

eeg_avg = zeros(matrix_size, matrix_size, n_timebins);  % to accumulate
valid_counts = 0;

for p = 18:n_participants
    filename = fullfile(strcat("D:\Dropbox\Internship\tasks_may2024\processing\eeg_aesthetics_", string(p)), ...
        "20Hz", strcat("ds_corr_", string(p)));
    loaded = load(filename);
    
    eeg_avg = eeg_avg + loaded.ds_corr;  % accumulate
...
%%=================== PREPARE INDEX MASK FOR VECTOR CORR ==================
% We only correlate the off‐diagonal elements. Build a logical mask 
% that is 0 on the diagonal, 1 elsewhere.
...
title('50ms RSA: Averaged EEG RDMs vs Style Models');
grid off;
hold off;
...
ylabel('RSA Correlation');
legend(legend_cat);
title('50ms RSA: Averaged EEG RDMs vs Category Models');
grid off;
hold off;
